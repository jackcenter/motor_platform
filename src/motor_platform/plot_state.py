...
def main():
    ...
    _, axes = plt.subplots(4, 1, figsize=(8.5, 6))
    ...
    __set_axis_settings("Position [rad]", axes[0])

    axes[1].scatter(state_times, reference_errors, label="position_error_rad")
    __set_axis_settings("Error [rad]", axes[1])

    plots.scatter_attribute(input_list, "voltage", axes[2])
    __set_axis_settings("Input [volts]", axes[2])

    plots.scatter_attribute(measurement_list, "encoder_1_pos", axes[3])
    __set_axis_settings("Position [counts]", axes[3])
